    ...
    try {
      ...
      if (authorization === `Bearer ${process.env.CRON_KEY}`) {
        ...
        const menosDeQuinceDias = moment()
          ...
          .startOf("day")
          .toDate();
        const menosDeQuinceDiasEnd = moment()
          .add(15, "days")
          .endOf("day")
          .toDate();
        try {
            ...
            .find(
              {
                "profile.fechaVencimientoSeguro": {
                  $lte: new Date(menosDeQuinceDiasEnd),
          ...
          for (const user of allUsers) {
            await nodemailerMailgun.sendMail({
              ...
              html: `<!DOCTYPE html>
                        ...
                        <body>
                            <p>Estimad@ ${user.profile.name} ${user.profile.lastName},</p>
